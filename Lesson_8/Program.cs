...
{
    if (row1 >= 0 && row1 < array.GetLength(0) && row2 >= 0 && row2 < array.GetLength(1) && row1 != row2)
        for (int j = 0; j < array.GetLength(1); j++)
        {
...
int[,] RemoveMinEl (int[,] array)
{
    int rows = array.GetLength(0);
    int columns = array.GetLength(1);
    int[,]ChanheArr = new int [rows - 1, columns - 1];
    int min = array[0,0];
    int minRow = 0;
    int minColumn = 0;
    for (int i = 0; i < rows; i++)
...
int[,] newArray = CreateRandom2dArray();
Show2dArray(newArray);
int [,] myArr = RemoveMinEl(newArray);
Show2dArray(myArr);
*/
